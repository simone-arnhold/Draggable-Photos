addEventListener("DOMContentLoaded", () => {
  ...
  photoContainers.forEach((photoContainer) => {
    ...
    var mainSpaceHeight =
      document.querySelector("#main-space").offsetHeight * 0.5

    // place default photocontainers at random
    ...
    frameSelector.addEventListener("click", () => {
      ...
      if (frameSelector.classList.contains("fa-solid")) {
        ...
        // frameSelector.style.transform = "scale(1.03)"
      } else {
        frameSelector.setAttribute("class", "fa-solid fa-square fa-2x")
        noFrameEnabled = false
        // frameSelector.style.transform = "scale(1.00)"
      }
    })

    //frame resizer
    function photoResize() {
      const photoResizerSelector = document.getElementById(
        "photo-resizer-selector"
      )
      photoResizerSelector.addEventListener("click", () => {
        photoResizerSelector.classList.toggle("selected")

        if (photoResizerSelector.classList.contains("selected")) {
          photoContainers.forEach((photoContainer) => {
            photoContainer.style.resize = "horizontal"
          })
        } else {
          photoContainers.forEach((photoContainer) => {
            photoContainer.style.resize = "none"
          })
        }
      })
    }
